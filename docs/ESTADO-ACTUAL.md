...
- **🔥 Prioridades:** **espacio 100% personal** del usuario (matriz Eisenhower). Solo muestra lo que el propio usuario escribe en cada cuadrante (`priority_items`); **NO** inyecta tareas ni instrucciones del sistema. KPIs/dona se basan en prioridades personales (activas / hacer ahora / planificar / completadas); sin panel de plazos. El **jefe (admin)** ve la matriz de cada funcionario en **solo lectura** (banner "puedes verlo pero no intervenir"); no puede editarla.
...
7. **HOY = fecha real local** (antes fija `2026-06-09`). Ventanas "próximos 3 días"/reuniones y cálculo de plazo usan `fechaMas(n)`. ⚠️ Las grillas de **Calendario/Gantt/Time-blocks siguen fijas a junio 2026** (limitación previa; revisar cuando cambie de mes).
8. **Tareas vs Instrucciones (separadas):** la pantalla **Tareas** excluye instrucciones (muestra tareas de proyecto + directas). El funcionario tiene nav **"📌 Mis instrucciones"** (`renderInstrucciones` rol-aware, sin botón de envío). Las **tareas se asocian a proyectos**; las **instrucciones** son requerimientos puntuales.
9. **Fechas:** las **instrucciones** se bloquean estrictamente a hoy en adelante (`min`=HOY + validación dura, no solo aviso). Las **tareas de proyecto** SÍ permiten fecha **retroactiva**, pero queda registrada en **Registro** (`dbAct` con "FECHA RETROACTIVA") para auditoría.
